...
import { egressPolicyForUrl } from './egress-policy.js';
...
function sourceHeaders(url, sources = {}) {
  const adapter = adapterForUrl(url);
  return { 'user-agent': 'rsshub-gateway/0.1', ...adapter.headers(sources[adapter.name]) };
}
...
export function createUpstreamClient({
  ...
  async function requestWithPolicy(url, { headers = {}, range, priority = 'foreground', galleryShard, timeout = totalTimeoutMs, source = sourceName(url), useProxy = true, allowTarget = false, recordResponseFailures = true, circuit = true } = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      while (true) {
        ...
        const requestHeaders = useProxy
          ? { ...sourceHeaders(current.toString(), sourceConfig), ...headers }
          : headers;
        ...
        try {
          let response;
          try {
            if (useProxy && egressPool && egressPolicyForUrl(current) === 'public') {
              try {
  ...
  async function fetchExternal(url, { headers = {}, range, timeout, priority = 'foreground', galleryShard, circuit = true } = {}) {
    const target = new URL(url);
    if (!isAllowedTarget(target)) throw new Error('external target is not allowed');
    try {
      onRequestPolicy?.({
        host: target.hostname.toLowerCase(),
        policy: egressPolicyForUrl(target),
      });
    } catch {
      // Diagnostics must never affect upstream requests.
    }
    return requestWithPolicy(target, { headers, range, priority, galleryShard, timeout: timeout ?? totalTimeoutMs, source: target.hostname.toLowerCase(), useProxy: true, allowTarget: true, circuit });
  }
