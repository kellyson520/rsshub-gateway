...
export const HOTLINK_REFERERS = Object.freeze({
  ...
  'fanza.co.jp': 'https://www.dmm.co.jp/',
  ...
  'dlsite.jp': 'https://www.dlsite.com/',
  'dlsite.com': 'https://www.dlsite.com/',
  'hembed.com': 'https://hanime1.me/',
  'hanime1.me': 'https://hanime1.me/',
  'chikubi.jp': 'https://chikubi.jp/',
  'wnacg.com': 'https://www.wnacg.com/',
  'wnacg.org': 'https://www.wnacg.com/',
  'sehuatang.net': 'https://www.sehuatang.net/',
  'sehuatang.org': 'https://www.sehuatang.net/',
  'kemono.su': 'https://kemono.su/',
  'coomer.su': 'https://coomer.su/',
  'skeb.jp': 'https://skeb.jp/',
...
export function rewriteCookedHtml(html, { baseUrl, secret }) {
  if (!html) return '';
  ...
  $('img,video,audio,source').each((_, element) => {
    for (const attribute of ['src', 'poster', 'data-orig-src', 'data-src']) {
      ...
      else if (value.startsWith('/')) value = `${LINUXDO_SITE_BASE}${value}`;
      try {
        $(element).attr(attribute, signedGatewayUrl(baseUrl, 'media', new URL(value).toString(), { secret, signedTargetMetadata: { egressScope: 'public', source: 'linuxdo' } }));
      } catch {
        // preserve
      }
    }
  ...
  $('a[href]').each((_, element) => {
    ...
    if (!href) return;
    if (href.startsWith('/')) href = `${LINUXDO_SITE_BASE}${href}`;
    try {
      if (isLinuxdoTopicTarget(href)) {
        $(element).attr('href', signedGatewayUrl(baseUrl, 'item', new URL(href).toString(), { secret, signedTargetMetadata: { egressScope: 'public', source: 'linuxdo' } }));
      }
    } catch {
      // preserve
    }
...
export const LINUXDO_READER_STYLE = `
:root {
  --bg-color: #f6f8fa;
  --card-bg: #ffffff;
  --text-main: #1f2328;
...
export function renderLinuxdoReaderPage({ topic = {}, baseUrl = '', secret }) {
  ...
  const originalUrl = `${LINUXDO_SITE_BASE}/t/${topic.slug || 'topic'}/${topic.id}`;
  ...
  if (mainPost.avatar_template) {
    ...
    const fullAvatar = rawAvatar.startsWith('http') ? rawAvatar : `${LINUXDO_SITE_BASE}${rawAvatar}`;
    try {
      avatarUrl = signedGatewayUrl(baseUrl, 'media', fullAvatar, { secret, signedTargetMetadata: { egressScope: 'public', source: 'linuxdo' } });
    } catch {
  ...
  const repliesHtml = replies.map((reply, idx) => {
    ...
    if (reply.avatar_template) {
      ...
      const full = raw.startsWith('http') ? raw : `${LINUXDO_SITE_BASE}${raw}`;
      try {
        replyAvatar = signedGatewayUrl(baseUrl, 'media', full, { secret, signedTargetMetadata: { egressScope: 'public', source: 'linuxdo' } });
      } catch {
        replyAvatar = full;
      }
    }
...
<head>
  ...
  <style>${LINUXDO_READER_STYLE}</style>
</head>
<body>
  <div class="container">
...
export function renderIwaraFeed({ username = '', kind = 'video', videos = [], selfUrl = '' } = {}) {
  const entries = videos.map((video) => {
    ...
    const link = isImage ? `${IWARA_SITE_BASE}/image/${video.id}` : iwaraVideoPageUrl(video);
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/" xmlns:media="http://search.yahoo.com/mrss/"><channel><title>${escapeXml(title)}</title><link>${escapeXml(IWARA_SITE_BASE)}/users/${escapeXml(username)}</link><description>Iwara ${escapeXml(kind)} by ${escapeXml(username)}</description>${entries}</channel></rss>`;
}
...
export const DEFAULT_EH_IMAGE_PRELOAD_COUNT = 1;
export const IMAGE_SIZES = '(min-width:1120px) 1120px, 100vw';
...
export function renderDocument(title, content, preloadImages = []) {
...
function renderReaderPagination($, url, baseUrl, secret, signedTargetMetadata) {
  const links = $('.gtb').first().find('a[href]').map((_, element) => {
    const href = resolveGatewayUrl(baseUrl, 'item', $(element).attr('href'), url, secret, signedTargetMetadata);
...
export function renderEhGalleryPage({ url, html, baseUrl, secret, signedTargetMetadata }) {
  ...
  const cover = parseThumbnailTile($('#gd1 > div').first().attr('style'), url, baseUrl, secret, signedTargetMetadata);
  const summary = cleanText($('.gtb .gpc').first().text());
  const thumbnails = $('#gdt > a').map((_, element) => {
    const anchor = $(element);
    const href = resolveGatewayUrl(baseUrl, 'item', anchor.attr('href'), url, secret, signedTargetMetadata);
    const tileElement = anchor.children('div').first();
    const tile = parseThumbnailTile(tileElement.attr('style'), url, baseUrl, secret, signedTargetMetadata);
  ...
  const header = `<section class="eh-gallery-info"><div>${cover
    ...
    : '<div class="eh-cover eh-cover-empty">暂无封面</div>'}</div><div><h1 class="eh-title">${escapeHtml(title)}</h1>${subtitle ? `<p class="eh-subtitle">${escapeHtml(subtitle)}</p>` : ''}${uploader ? `<p class="eh-byline">上传者：${escapeHtml(uploader)}</p>` : ''}${category ? `<p class="eh-byline">分类：${escapeHtml(category)}</p>` : ''}${renderReaderRating($)}${renderReaderMetadata($)}${renderReaderTags($)}</div></section>`;
  const body = `${header}${summary ? `<p class="eh-summary">${escapeHtml(summary)}</p>` : ''}${renderReaderPagination($, url, baseUrl, secret, signedTargetMetadata)}<section class="eh-grid" aria-label="画廊预览">${thumbnails || '<p class="eh-summary">暂无预览图</p>'}</section>`;
...
export function extractEhImagePage({ url, html, baseUrl, secret, pageNumber, signedTargetMetadata }) {
  ...
  const media = signedGatewayUrl(baseUrl, 'media', mediaTarget, secret, signedTargetMetadata);
...
export function renderEhImageSequence({
  ...
  const renderedPages = pages.map((page) => ({
    ...
    media: page.media || (page.mediaTarget && baseUrl
      ? signedGatewayUrl(baseUrl, 'media', page.mediaTarget, secret, signedTargetMetadata)
      : ''),
...
export function renderEhImagePage({ url, html, baseUrl, secret, signedTargetMetadata }) {
  const $ = cheerio.load(html, { decodeEntities: false });
  ...
  const previous = resolveGatewayUrl(baseUrl, 'item', previousAnchor.attr('href'), url, secret, signedTargetMetadata);
  const next = resolveGatewayUrl(baseUrl, 'item', nextAnchor.attr('href'), url, secret, signedTargetMetadata);
  const navigation = [
...
export function renderGenericReaderPage({ url, html, baseUrl, secret, signedTargetMetadata }) {
  ...
  $('img[src],video[src],video[poster],audio[src],source[src],source[poster]').each((_, element) => {
    for (const attribute of ['src', 'poster']) {
      ...
      if (!value) continue;
      try {
        $(element).attr(attribute, signedGatewayUrl(baseUrl, 'media', new URL(value, url).toString(), secret, signedTargetMetadata));
      } catch {
  ...
  $('a[href]').each((_, element) => {
    ...
    try {
      if (value) $(element).attr('href', signedGatewayUrl(baseUrl, 'item', new URL(value, url).toString(), secret, signedTargetMetadata));
    } catch {
...
export function renderUnavailablePage({ url, title, message, baseUrl, secret, signedTargetMetadata }) {
  const sourceUrl = signedGatewayUrl(baseUrl, 'item', url, secret, signedTargetMetadata);
