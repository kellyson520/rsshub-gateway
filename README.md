...
- **Dynamic Microservice Frameworks, Gateways & Server Utilities (`src/fetcher-server.js`, `src/server.js`, `src/request-handler.js`, `src/gallery-benchmark.js`, `src/http-utils.js`)**:
  ...
  - `renderReaderPage(opts)`, `renderEhGalleryPage(opts)`, `renderEhImagePage(opts)`, `renderEhImageSequence(opts)`, `renderGenericReaderPage(opts)`, `renderUnavailablePage(opts)`, `extractEhImagePage(opts)`, `renderDocument(title, body, preloads)`, `READER_CSS`, `DEFAULT_EH_IMAGE_PRELOAD_COUNT`, `IMAGE_SIZES`: Full-featured distraction-free reader engine supporting responsive E-Hentai gallery preview grids, continuous horizontal/vertical image viewers, sanitization, and fallback diagnostics.
