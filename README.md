...
- **Dynamic Microservice Frameworks, Gateways & Server Utilities (`src/fetcher-server.js`, `src/server.js`, `src/request-handler.js`, `src/gallery-benchmark.js`, `src/http-utils.js`)**:
  ...
  - `resolveSourceMode(source, config)`, `defaultAdapter`, `xHeaders(config, opts)`, `pixivHeaders(config, opts)`, `adultMediaHeaders(opts)`, `isXReaderUnavailable(html, parser)`, `isTelegramChannelPostUrl(url, hosts)`, `telegramReaderTarget(url, hosts)`, `isInstagramReaderUnavailable(html, parser)`, `isAdultMediaChallenge(res)`: Universal multi-source adapter mode resolver, default fallback adapter, X (Twitter) / Pixiv / Adult Media request header and cookie builders, X / Instagram login restriction detector, Telegram embed target builder, and adult media Cloudflare challenge detector.
