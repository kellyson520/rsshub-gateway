...
- **Dynamic Microservice Frameworks, Gateways & Server Utilities (`src/fetcher-server.js`, `src/server.js`, `src/request-handler.js`, `src/gallery-benchmark.js`, `src/http-utils.js`)**:
  ...
  - `createLeaseProxy(opts)`, `isLeaseComplete(lease)`, `rejectConnect(socket, status, msg)`, `formatConnectHeader(host, port)`, `DEFAULT_LEASE_UPSTREAM_PROXY_HOST`, `DEFAULT_LEASE_UPSTREAM_PROXY_PORT`, `DEFAULT_LEASE_FAILURES_CAP`: Zero-leakage one-time download lease HTTP CONNECT proxy tunnel, IP rate limiter, handshake chunk boundary accumulator, 405/407/502 rejection formatter, and upstream proxy limits.
