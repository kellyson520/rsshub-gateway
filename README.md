...
Video range requests that miss the gateway cache start a background full-file fill, so the first seek seeds the cache and later ranges (including parallel ones) are served from disk. `GET /_gateway/media/:token?download=1` adds an attachment `Content-Disposition`, and `GET /_gateway/media/:token?chunks=N` returns a JSON manifest of `N` signed chunk URLs; each `/_gateway/chunk/:token` URL serves its byte range (206) independently, so IDM/aria2-style downloaders can pull a file with several parallel connections.
...
The signed-target allowlist covers the media CDNs used by RSSHub feeds (imgur, Discord, Reddit, YouTube thumbnails, Flickr, MyAnimeList, AniList, TMDB, Steam, eBay, postimg, GitHub raw/camo, weserv/wsrv image resizers) on top of the site hosts, so `transformFeed` can rewrite enclosures and images from arbitrary RSSHub routes to the accelerated gateway media path.
