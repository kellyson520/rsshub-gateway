...
- **Dynamic Microservice Frameworks, Gateways & Server Utilities (`src/fetcher-server.js`, `src/server.js`, `src/request-handler.js`, `src/gallery-benchmark.js`, `src/http-utils.js`)**:
  ...
  - `createLeaseStore(opts)`, `createLeaseProxy(opts)`, `isLeaseComplete(lease)`, `rejectConnect(socket, status, msg)`, `formatConnectHeader(host, port)`, `DEFAULT_LEASE_UPSTREAM_PROXY_HOST`, `DEFAULT_LEASE_UPSTREAM_PROXY_PORT`, `DEFAULT_LEASE_FAILURES_CAP`: One-time download lease store (constant-time verification, expiry sweeps, public views), zero-leakage HTTP CONNECT proxy tunnel, IP rate limiter, and handshake chunk boundary accumulator.
  ...
  - `createEgressPool(opts)`, `poolError(msg, code)`, `DEFAULT_EGRESS_SUCCESS_RAMP_AFTER`, `DEFAULT_EGRESS_COOLDOWN_MS`, `DEFAULT_EGRESS_BACKGROUND_RESERVE_PER_LANE`, `DEFAULT_EGRESS_EWMA_ALPHA`, `DEFAULT_EGRESS_MAX_LATENCY_SAMPLE_MS`: High-concurrency egress proxy lane rate limiting, EWMA latency tracking, dynamic concurrency ramping/backoff, and typed pool exception factory.
  - `createDownloadSessionStore(opts)`, `buildDownloadSession(opts)`, `restoreDownloadSessionRecord(record)`, `DOWNLOAD_SESSION_VERSION`: Pure state multi-threaded download session builder, atomic file-backed session persistence store, chunk progress tracker, and projection deserializer.
  ...
  - `isIwaraVideoTarget(url)`, `iwaraVideoId(url)`, `iwaraThumbnailUrl(id, idx)`, `iwaraVideoPageUrl(video, base)`, `selectIwaraVariant(variants)`, `fetchIwaraUser(fetch, name, opts)`, `fetchIwaraVideos(fetch, id, opts)`, `fetchIwaraVideoDetail(fetch, id, opts)`, `refreshIwaraAccessToken(fetch, token, opts)`, `resolveIwaraVideoStream(fetch, detail)`: Iwara video pipeline — REST API endpoints, profile/autocomplete lookups, JWT token refreshes, and video stream bitrate resolution.
  ...
  - `resolveSourceMode(source, config)`, `defaultAdapter`, `matchAdapter(url, list, fallback)`, `getAdapterSourceNames(list)`, `isKnownAdapterTarget(url, list, fallback)`, `xHeaders(config, opts)`, `pixivHeaders(config, opts)`, `adultMediaHeaders(opts)`, `isXReaderUnavailable(html, parser)`, `isTelegramChannelPostUrl(url, hosts)`, `telegramReaderTarget(url, hosts)`, `isInstagramReaderUnavailable(html, parser)`, `isAdultMediaChallenge(res)`: Universal multi-source adapter mode resolver, pluggable adapter matchers and source ecosystem inspectors, X (Twitter) / Pixiv / Adult Media request header and cookie builders, X / Instagram login restriction detector, Telegram embed target builder, and adult media Cloudflare challenge detector.
  ...
  - `createRequestService(opts)`: Unified request dispatcher facade orchestrating browser TLS workers and upstream pool dispatchers with fallback telemetry.
  ...
  - `resolveGatewayOptions(opts, env)`, `resolveSlowSourceThresholdMs(raw, fallback)`, `parseFeedPrefetchPaths(raw, dedupeFn)`, `parseBooleanOption(raw, fallback)`: Canonical runtime gateway configuration options resolver with strict bounded validation, fallback hierarchies, and type coercion.
  - `createMediaPrefetchQueue(opts)`, `createFeedPrefetchQueue(opts)`, `DEFAULT_MEDIA_PREFETCH_INITIAL_CONCURRENCY`, `DEFAULT_MEDIA_PREFETCH_MIN_CONCURRENCY`, `DEFAULT_MEDIA_PREFETCH_MAX_CONCURRENCY`, `DEFAULT_MEDIA_PREFETCH_PER_ORIGIN_CONCURRENCY`, `DEFAULT_FEED_PREFETCH_INTERVAL_MS`, `DEFAULT_FEED_PREFETCH_CONCURRENCY`: Autonomous media prefetch task scheduler with dynamic ramp/backoff and feed warmup queue with exponential error retries and pause toggles.
  - `createMihomoEgressAdapter(opts)`, `laneId(index)`, `laneGroup(index)`, `sessionLaneId(index)`, `sessionLaneGroup(index)`, `listenerUrl(baseUrl, index, port)`: Mihomo controller orchestration client, multi-scope health prober, dedicated session slots, and local proxy listener URL builders.
  ...
  - `createLeaseBackfillQueue(opts)`, `DEFAULT_LEASE_BACKFILL_MAX_CONCURRENCY`, `DEFAULT_LEASE_BACKFILL_EVICTION_BUDGET`, `DEFAULT_LEASE_BACKFILL_VIDEO_CACHE_MAX_FILE_BYTES`: One-time lease download background video slice cache warmer, concurrency throttler, and memory budget limits.
  ...
  - `buildBrowserFetchPayload(url, opts, impersonate, maxBody)`, `browserFetchLineError(msg, opts)`, `browserRequestTimeoutMs(timeout)`, `messageToResponse(msg)`: Python TLS-fingerprint subprocess message payload serializer, typed upstream error wrapper, and response adapter.
  ...
  - `isPublicEgressTarget(url, hosts)`, `isPublicRequestTarget(url, reqHosts)`, `egressPolicyForUrl(url, hosts)`, `egressPolicyForRequest(url, opts)`, `resolveEgressPublicHosts(env, fallback)`, `resolveEgressPublicRequestHosts(reqEnv, pubEnv, fallback)`, `EGRESS_POLICIES`, `DEFAULT_PUBLIC_HOSTS`, `DEFAULT_PUBLIC_REQUEST_HOSTS`: Standard egress policy resolver categorizing upstream URLs into shared pool or sticky session lanes, and environment host list normalizers.
  - `fingerprintFor(source, creds, secret)`, `chooseLane(fp, lanes, unhealthy)`, `normalizedLaneIds(lanes)`, `normalizedCredentials(creds)`, `isValidAffinityRecord(rec, now, maxAge)`, `DEFAULT_SESSION_AFFINITY_VERSION`, `DEFAULT_SESSION_AFFINITY_MAX_AGE_MS`: Deterministic session-affinity routing, HMAC credential fingerprinting, and healthy lane selection.
  - `createPoller(opts)`, `DEFAULT_POLLER_INTERVAL_MS`, `DEFAULT_POLLER_JITTER_RATIO`, `MIN_TASK_INTERVAL_MS`, `MAX_JITTER_RATIO`: Unified background polling task scheduler with individual jitter and concurrency-safe non-overlapping execution.
  ...
  - `createBrowserFetchClient(opts)`, `createFetchdCompat(opts)`, `createFetchdClient(opts)`, `fetchdJson(fetchdFetch, url, opts)`, `buildBrowserFetchPayload(url, opts, imp, max)`, `browserRequestTimeoutMs(timeout)`, `messageToResponse(msg)`, `browserFetchLineError(msg, opts)`, `DEFAULT_FETCHD_BASE_URL`, `DEFAULT_FETCHD_TIMEOUT_MS`, `MAX_FETCHD_TIMEOUT_MS`, `FETCHD_TIMEOUT_SLACK_MS`: Python subprocess worker & sidecar browser fetch transport pipeline with auto-spawn, line protocol decoding, process crash recovery and HTTP fallback routing.
  - `createUpstreamClient(opts)`, `sourceHeaders(url, sources, opts)`, `responseWithLease(res, lease)`, `parseRetryAfter(res, max)`, `upstreamRetryDelay(attempt, first, rest)`, `isAuthenticationChallenge(res, url, custom)`, `isAuthenticationRedirect(res)`: Enterprise upstream HTTP client orchestrating retry backoff, circuit breaking, manual redirect validation, session escalation and adaptive egress pool leasing.
  ...
  - `matchesFeedFilters(item, filters)`, `rewriteFeedHtml(html, opts, parser)`: Feed transformation keyword/author blacklist evaluator and streaming HTML image/media/link tag signer and rewriter.
  ...
  - `createMediaTransport(opts)`, `unavailableMediaResponse()`: Unified media acceleration transport layer orchestrating Range streaming, image variant transformations, video chunk slices, and memory/disk caching.
  - `createResponseCache(opts)`, `canonicalUrl(url)`, `normalizedNamespace(ns)`, `cacheKeyFor(url, kind, ns)`, `normalizeCacheHeaders(headers)`, `normalizeCacheBody(body)`, `resultFromCacheEntry(entry, body, state)`, `DEFAULT_CACHE_TTL_SECONDS`, `DEFAULT_CACHE_MAX_BYTES`, `DEFAULT_EVICTION_PRIORITY`: Tiered disk and memory cache engine with atomic index persistence, LRU/kind-weighted eviction, in-flight request coalescing, Range stream readers, stale-if-error fallback, and namespace isolation.
  - `CACHE_INDEX_VERSION`, `CACHE_BODY_EXTENSION`, `CACHE_BODY_PATTERN`, `isValidCacheIndexRecord(record)`, `isCacheBodyFile(filename)`, `cacheBodyFile(key)`: Disk cache layout primitives — body filename generator, body file pattern validator, and index record integrity checker.
  - `isValidChunkRecord(chunk)`, `isValidSessionRecord(session, now)`, `CHUNK_STATUSES`, `DEFAULT_DOWNLOAD_SESSION_TTL_MS`, `DEFAULT_MAX_DOWNLOAD_SESSIONS`: Multi-threaded download session chunk progress validation and lifecycle storage contracts.
  ...
  - `createSessionAffinity(opts)`, `isValidAffinityRecord(record, now, maxAge)`, `chooseLane(fingerprint, lanes, unhealthyLanes)`, `fingerprintFor(source, creds, secret)`, `normalizedLaneIds(ids)`, `normalizedCredentials(creds)`, `proxyIdentityHash(id)`: Deterministic hashing session affinity router with persistent atomic state and automatic unhealthy lane failover migration.
  - `discoverEhGallery(opts)`, `resolveForegroundEhPage(opts)`, `prefetchEhGallery(opts)`, `initialEhGalleryManifest(opts)`: High-performance concurrent E-Hentai gallery crawling & bounded foreground deadline image resolution pipeline.
  - `renderReaderPage(opts)`, `renderLinuxdoReaderPage(opts)`, `renderIwaraReaderPage(opts)`, `renderIwaraFeed(opts)`, `rewriteCookedHtml(html, opts)`, `renderEhGalleryPage(opts)`, `renderEhImagePage(opts)`, `renderEhImageSequence(opts)`, `renderGenericReaderPage(opts)`, `renderUnavailablePage(opts)`, `extractEhImagePage(opts)`, `renderDocument(title, body, preloads)`, `READER_CSS`, `LINUXDO_READER_STYLE`, `DEFAULT_EH_IMAGE_PRELOAD_COUNT`, `IMAGE_SIZES`: Full-featured distraction-free reader engine supporting responsive LinuxDo topic forum discussions, Iwara video/image players, E-Hentai gallery preview grids, continuous horizontal/vertical image viewers, HTML sanitization, and fallback diagnostics.
  ...
  - `createDispatcher(opts)`, `compilePattern(routeId)`, `normalizeRoute(raw)`, `matchSegments(pattern, segments)`, `matchRouteList(routes, path)`, `registerRouteEntries(list, entries)`, `unregisterRouteEntries(list, ids)`, `buildSidecarFetchPayload(route, params, opts)`, `sidecarUrl(backend)`: Zero-overhead dynamic segment route dispatcher, YAML route configuration parser, runtime route registration, and Sidecar backend RPC client.
