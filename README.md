...
- **Dynamic Microservice Frameworks, Gateways & Server Utilities (`src/fetcher-server.js`, `src/server.js`, `src/request-handler.js`, `src/gallery-benchmark.js`, `src/http-utils.js`)**:
  ...
  - `isPublicEgressTarget(url, hosts)`, `isPublicRequestTarget(url, reqHosts)`, `egressPolicyForUrl(url, hosts)`, `egressPolicyForRequest(url, opts)`, `EGRESS_POLICIES`, `DEFAULT_PUBLIC_HOSTS`, `DEFAULT_PUBLIC_REQUEST_HOSTS`: Standard egress policy resolver categorizing upstream URLs into shared pool or sticky session lanes.
