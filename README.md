...
- **Dynamic Microservice Frameworks, Gateways & Server Utilities (`src/fetcher-server.js`, `src/server.js`, `src/request-handler.js`, `src/gallery-benchmark.js`, `src/http-utils.js`)**:
  ...
  - `createFetchdClient(opts)`, `fetchdJson(fetchdFetch, url, opts)`, `DEFAULT_FETCHD_BASE_URL`, `DEFAULT_FETCHD_TIMEOUT_MS`, `MAX_FETCHD_TIMEOUT_MS`, `FETCHD_TIMEOUT_SLACK_MS`: Headless browser-fetch RPC client with Base64 payload decoding, bounded deadline slack and upstream retry mapping.
