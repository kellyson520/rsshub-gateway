...
import { createSteamFetcher } from './fetcher.js';

const PORT = Number.parseInt(process.env.FETCHER_PORT || '', 10) || 8026;

async function main() {
  const browserClient = createBrowserFetchClient();

  const fetcher = createSteamFetcher({
    fetchExternal: async (url) => {
      ...
      try {
        const res = await fetch(url, {
          headers: {
            'user-agent': 'rsshub-gateway-steam/1.0',
            'accept': 'application/json',
          },
        });
        if (res.ok) {
  ...
  await listen(server, PORT, '0.0.0.0', 'fetcher_steam');
