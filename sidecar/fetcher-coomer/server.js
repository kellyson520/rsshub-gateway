...
import { createCoomerFetcher } from './fetcher.js';

const PORT = Number.parseInt(process.env.FETCHER_PORT || '', '10') || 8010;
...
const ROUTE_IDS = ['/coomer/:source?/:id?'];
...
async function main() {
  ...
  await listen(server, PORT, '0.0.0.0', 'fetcher_coomer');
...
if (process.argv[1] && new URL(`file://${process.argv[1]}`).pathname === new URL(import.meta.url).pathname) {
  main().catch((error) => {
    process.stderr.write(`fetcher-coomer failed to start: ${error.stack}\n`);
    process.exit(1);
  });
}
