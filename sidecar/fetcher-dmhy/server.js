...
import { createDmhyFetcher } from './fetcher.js';

const PORT = Number.parseInt(process.env.FETCHER_PORT || '', 10) || 8024;
...
async function main() {
  ...
  const fetcher = createDmhyFetcher({
    fetchHtml: async (url) => {
      try {
  ...
  await listen(server, PORT, '0.0.0.0', 'fetcher_dmhy');
