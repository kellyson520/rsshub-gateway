...
test('canonical system gateway configuration default constants are defined and exported', async () => {
  const {
    DEFAULT_CACHE_ROOT,
  ...
  assert.equal(DEFAULT_EH_PREFETCH_MAX_CONCURRENCY, 36);
  assert.equal(DEFAULT_EH_MAX_PREFETCH_PAGES, 300);
  assert.equal(DEFAULT_EH_MEDIA_PREFETCH_CONCURRENCY, 6);
  assert.equal(DEFAULT_EH_MEDIA_PREFETCH_MIN_CONCURRENCY, 3);
  assert.equal(DEFAULT_EH_MEDIA_PREFETCH_MAX_CONCURRENCY, 12);
  assert.equal(DEFAULT_EH_MEDIA_PREFETCH_PER_ORIGIN, 2);
  assert.equal(DEFAULT_EH_MEDIA_FOREGROUND_WARM_COUNT, 8);
  assert.equal(DEFAULT_EH_MEDIA_FOREGROUND_WARM_CONCURRENCY, 8);
  assert.equal(DEFAULT_EH_FIRST_PAINT_COUNT, 1);
  assert.equal(DEFAULT_EGRESS_MIN_CONCURRENCY_PER_LANE, 3);
  assert.equal(DEFAULT_EGRESS_MAX_CONCURRENCY_PER_LANE, 6);
  assert.equal(DEFAULT_EGRESS_MAX_TOTAL_CONCURRENCY, 48);
  assert.equal(DEFAULT_EGRESS_REFRESH_INTERVAL_MS, 60000);
  assert.equal(DEFAULT_MEDIA_CACHE_MAX_FILE_BYTES, 32 * 1024 ** 2);
  assert.equal(DEFAULT_VIDEO_CACHE_MAX_FILE_BYTES, 256 * 1024 ** 2);
  assert.equal(DEFAULT_MEDIA_BROWSER_CACHE_SECONDS, 300);
  assert.equal(DEFAULT_VIDEO_PREFETCH_CONCURRENCY, 4);
  ...
  const { positiveNumber, sourceHeaders } = await import('../src/http-utils.js');
  ...
  const testHdrs = sourceHeaders('https://example.com/test', {}, {
    adapterFor: () => ({ name: 'test', headers: () => ({ 'x-custom': 'val' }) }),
  });
  assert.equal(testHdrs['user-agent'], 'rsshub-gateway/0.1');
  assert.equal(testHdrs['x-custom'], 'val');
});
