...
<div class="relative">
  <div class="thumbnail group">
    <a href="https://missav.live/hrsm-156" alt="hrsm-156">
    ...
    <div class="my-2 text-sm text-nord4 truncate"><a href="https://missav.live/hrsm-156">「日常」：一位單身OL的日常生活</a></div>
  </div>
  <div class="thumbnail group">
    <a href="https://missav.live/ebwh-353" alt="ebwh-353">
...
test('parses missav grid group cards', () => {
  ...
  assert.equal(items[0].url, 'https://missav.live/hrsm-156');
...
test('maps the new route target', () => {
  assert.equal(missavTarget('/missav/new/:page?', {}).url, 'https://missav.live/new');
  assert.equal(missavTarget('/missav/new/:page?', { page: '3' }).url, 'https://missav.live/new?page=3');
  assert.equal(missavTarget('/missav/search/:keyword', { keyword: '三上悠亞' }).url, 'https://missav.live/search/%E4%B8%89%E4%B8%8A%E6%82%A0%E4%BA%9E');
...
test('fetcher returns rssXml with covers and video sources', async () => {
  ...
  assert.ok(result.rssXml.includes('https://missav.live/hrsm-156'));
...
test('fetcher serves a missav search feed', async () => {
  ...
  assert.ok(result.rssXml.includes('「日常」：一位單身OL的日常生活'));
  assert.deepEqual(fetched, ['https://missav.live/search/%E4%B8%89%E4%B8%8A%E6%82%A0%E4%BA%9E']);
});
